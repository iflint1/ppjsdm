...
steps <- 100000
beta0 <- c(4.5, 1.5)
alpha <- cbind(c(0, 2.5), c(2.5, 0))

covariates <- list(temperature = function(x, y) x)
beta <- matrix(c(0, 0), nrow = ntypes, ncol = 1)
ndummy <- 1000
...
se <- matrix(NA, nrow = nreplications, ncol = ncol(is_in))
se_1 <- matrix(NA, nrow = nreplications, ncol = ncol(is_in_1))
se_2 <- matrix(NA, nrow = nreplications, ncol = ncol(is_in_2))
...
se_estimates <-colMeans(se, na.rm = TRUE)
...
se_estimates_1 <-colMeans(se_1, na.rm = TRUE)
...
se_estimates_2 <-colMeans(se_2, na.rm = TRUE)
