...
* $\lambda_i$ is the intensity of species $i$ if the $\beta_k$ and $\alpha_{i,j}$ were to all be zero;
...
* $\alpha_{i,j}$ is the repulsion parameter between species $i$ and $j$;
...
* $r_{i,j}$ are interaction radii which denote the typical distance within which species $i$ and $j$ interact;
...
To begin with, you can control the radius of interaction (which correspond to $r_{i,j}$ in the model).
...
It is also possible to change the number of closest individuals that are taken into account, via the `saturation` parameter (which corresponds to $N$ in the model).
...
There are also various kinds of smoothing available via the `model` parameter (which correspond to $\varphi_{r_{i,j}}$ in the model).
