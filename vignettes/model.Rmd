...
```{r}
plot_pap <- function(type, model, medium_range_model, alpha, 
                     lambda, beta, gamma, covariates, short_range, medium_range, long_range, saturation) {
  configuration <- rgibbs(window = window, 
                        alpha = alpha, 
                        lambda = lambda, 
...
```{r}
plot_pap(1, model, medium_range_model, alpha, lambda, beta, gamma, 
         covariates, short_range, medium_range, long_range, saturation) 
```
...
ncovariates <- 0
lambda <- c(10, 10)
alpha <- cbind(c(-1, .7), c(.7, -0.5))
gamma <- cbind(c(0.5, -1), c(-1, 0.5))
beta <- matrix(0, nspecies, ncovariates)
covariates <- list()
model <- "linear"
medium_range_model <- "square_exponential"
short_range <- cbind(c(0.1, 0), c(0, 0.2))
medium_range <- cbind(c(0.1, 0), c(0, 0.2))
long_range <- cbind(c(0.2, 0.1), c(0.1, 0.4))
saturation <- 2
window <- Rectangle_window()
spatstat_window <- owin()
```
