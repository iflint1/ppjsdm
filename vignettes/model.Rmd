...
+\sum_{j=1}^p\alpha_{i,j}\frac{1}{|\omega_i^{(N)}|}\sum_{z\in\omega_i^{(N)}}\varphi_{r_{i,j}}(\|z-x\|)\biggr).$$
...
* $|\omega_i^{(N)}|$ is the number of points in the above configuration;
...
Writing a function to plot the configuration and the Papangelou conditional intensity will be useful.
