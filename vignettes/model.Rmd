...
For a point configuration $\omega$, a location in space $x$, and a species index $i$, the Papangelou conditional intensity $\pi$ is such that $\pi((x,i),\omega)\dot\Delta x$ is the probability of finding a point in the infinitesimal volume $\Delta x$ around $x$, conditional on $\omega$ outside of $\Delta x$.
...
z <- outer(x_axis, y_axis, function(x, y) {
  compute_papangelou(configuration, x, y, 1, model, alpha, 
                     lambda, coefs, covariates, radius, saturation)
})
```

The Papangelou conditional intensity is equal to the intensity over the whole region:
...
In other words, the associated point process is a Poisson point process.
...
```{r}
plot_pap(configuration, 1, model, alpha, lambda, coefs, covariates, radius, saturation) 
```
...
As an illustration, here is a plot of an extremely attractive version of the square exponential version of the model.
...
It is best to provide the covariates as `spatstat::im` objects, and it is usually easy to convert to that format.
