...
```{r, out.width = '1.3 \\textwidth', fig.height = 4, fig.align = 'center', fig}
...
```{r}
average_height <- matrix(c(100, 0, 0, 50), ncol = 2, nrow = 2)

traits <- list(average_height = average_height)
```
...
```{r}
ppjsdm::gibbsm(configuration, traits = traits)
```
...
traits <- list(average_height = ret$average_height)
window <- ppjsdm::Rectangle_window(c(0, 100), c(0, 50))
radii <- matrix(5, number_of_species, number_of_species)
```
