...
alpha <- matrix(cbind(c(-1.2, 0.4, -0.5), c(0.4, -0.9, -1.3), c(-0.5, -1.3, 0.5)), 
                nspecies, 
                nspecies)
gamma <- matrix(cbind(c(0.2, 0.4, -0.7), c(0.4, 0.5, -0.9), c(-0.7, -0.9, -0.9)), 
                nspecies, 
                nspecies)
short_range <- matrix(0.005, nspecies, nspecies)
medium_range <- matrix(0.01, nspecies, nspecies)
long_range <- matrix(0.015, nspecies, nspecies)
beta0 <- rep(6, nspecies)

configuration <- ppjsdm::rgibbs(beta0 = beta0, 
                                alpha = alpha,
                                gamma = gamma,
                                model = model,
...
<!-- ```{r} -->
<!-- configuration <- ppjsdm::rgibbs(lambda = exp(c(5, 5, 5)), -->
<!--                                 alpha = alpha, -->
<!--                                 model = model, -->
<!--                                 short_range = short_range, -->
<!--                                 medium_range = medium_range, -->
