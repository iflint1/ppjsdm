...
long_range <- matrix(0.02, nspecies, nspecies)
...
for(i in seq_len(nspecies)) {
  for(j in seq_len(nspecies)) {
    if(i == j) {
      alpha[i, j] <- gamma[1] + beta %*% (traits_matrix[, i] / traits_maximum)
    } else {
      alpha[i, j] <- gamma[2] + delta %*% (abs(traits_matrix[, i] - traits_matrix[, j]) / traits_maximum_difference)
    }
  }
}
print(alpha)
...
configurations <- append(ppjsdm::rgibbs(lambda = c(50, 50), 
                                        ...
                                        types = c("type1", "type2"), 
                                        model = model, 
                                        short_range = short_range[c(1, 2), c(1, 2)],
                                        drop = FALSE),
                                      
                         ppjsdm::rgibbs(lambda = c(50, 50, 50), 
                                        alpha = alpha,
                                        nsim = 100,
