...
inline SEXP rppp_helper(const ppjsdm::Window& window, const Rcpp::NumericVector& lambda, R_xlen_t nsim, Rcpp::CharacterVector types, bool drop) {
  Rcpp::List samples(nsim);

  for(R_xlen_t i(0); i < nsim; ++i) {
    const auto sample(ppjsdm::rppp_single<ppjsdm::Configuration_wrapper>(window, lambda));
