...
public:
  ...
  template<typename X, typename Y, typename T>
  [[nodiscard]] inline Rcpp::NumericVector compute(const X& x, const Y& y, const T& types_vector, Rcpp::NumericVector location, [[maybe_unused]] R_xlen_t type, R_xlen_t number_types, R_xlen_t number_points) const {
    Rcpp::NumericVector delta_dispersion(number_types);
    for(R_xlen_t i{0}; i < number_points; ++i) {
      const auto type_i{types_vector[i] - 1};
  ...
  [[nodiscard]] inline Rcpp::NumericVector compute(const X& x, const Y& y, const T& types_vector, Rcpp::NumericVector location, R_xlen_t type, R_xlen_t number_types, R_xlen_t number_points) const {
    ...
    for(R_xlen_t i{0}; i < number_types; ++i) {
      if(i == type) {
        ...
        delta_dispersion[i] = d + compute_dispersion(copy_x, copy_y, copy_types, i, i, number_points + 1);
      } else {
        ...
        delta_dispersion[i] = 0.5 * (d + compute_dispersion(copy_x, copy_y, copy_types, i, type, number_points + 1) + compute_dispersion(copy_x, copy_y, copy_types, type, i, number_points + 1));
      }
    }
    return delta_dispersion;
  }
