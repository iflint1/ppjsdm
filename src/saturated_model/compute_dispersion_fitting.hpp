    ...
    if(static_cast<decltype(max_points_by_type)>(varphi.get_saturation()) >= max_points_by_type + 1) {
      for(size_t i(0); i < configuration_size; ++i) {
        ...
        for(size_t j(0); j < i; ++j) {
          ...
          AbstractDispersion::template update_count<1000>(varphi, count_vector[i][get_type(configuration[j])],
                                                                                  configuration[i], configuration[j]);
          AbstractDispersion::template update_count<1000>(varphi, count_vector[j][get_type(configuration[i])],
                                                                                  configuration[i], configuration[j]);
        }
      }
...
#pragma omp for nowait
      for(decltype(size(other_configuration)) i = 0; i < size(other_configuration); ++i) {
        ...
        for(size_t j(0); j < size(configuration); ++j) {
          AbstractDispersion::template update_count<1000>(varphi, count_point[get_type(configuration[j])],
