...
std::pair<bool, Configuration> update_LU_and_check_coalescence(Chain& chain, const Model& model, R_xlen_t number_types) {
  ...
  chain.iterate_forward_in_time([&points_not_in_L, &L](auto&& p, bool is_birth, auto mark) {
    if(is_birth) {
      const auto papangelou_L(model.compute_papangelou(L, p, number_types));
      if(papangelou_L > mark) {
        const auto papangelou_U(model.compute_papangelou(points_not_in_L, p, number_types) * papangelou_L);
...
inline auto simulate_coupling_from_the_past(const Model& model, const Window& window, R_xlen_t number_types) {
  ...
  const auto T0(Z.extend_until_T0(detail::get_integral_of_dominating_intensity(dominating_intensity, window), window, number_types));
  while(true) {
    ...
    Z.extend_backwards(T0, detail::get_integral_of_dominating_intensity(dominating_intensity, window), window, number_types);
  }
}
