...
inline auto get_integral_of_dominating_intensity(const Lambda& dominating_intensity, const Window& window, R_xlen_t number_types) {
  ...
  for(R_xlen_t i(0); i < number_types; ++i) {
    integral += dominating_intensity[i] * window.volume();
  }
...
inline auto simulate_coupling_from_the_past(const Model& model, const Window& window, R_xlen_t number_types) {
  ...
  const auto T0(Z.extend_until_T0(detail::get_integral_of_dominating_intensity(dominating_intensity, window, number_types), window, number_types));
  while(true) {
    ...
    Z.extend_backwards(T0, detail::get_integral_of_dominating_intensity(dominating_intensity, window, number_types), window, number_types);
  }
}
