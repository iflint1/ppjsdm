...
get_parameters_from_fit <- function(fit) {
  ...
  log_lambda_indices <- grep("log_lambda", names(coefs), value = TRUE)
  nspecies <- length(log_lambda_indices)

  lambda <- exp(coefs[log_lambda_indices])

  alpha <- matrix(NA, nspecies, nspecies)
  for(i in seq_len(nspecies)) {
    for(j in seq_len(nspecies)) {
      alpha[i, j] <- coefs[paste0("alpha_", min(i, j), "_", max(i, j))]
    }
  }
  ...
  for(i in seq_len(nspecies)) {
    for(j in seq_len(nspecies)) {
      gamma[i, j] <- coefs[paste0("gamma_", min(i, j), "_", max(i, j))]
    }
  }
