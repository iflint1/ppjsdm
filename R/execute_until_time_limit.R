#' Execute a function on a list of objects, using educated guesses to execute as many calls
...
#' @param unit Unit used to measure the time limit (hours, minutes, seconds, etc).
#'
#' @importFrom stats sd
#' @keywords internal
execute_until_time_limit <- function(objects, func, time_limit = Inf, unit = "hours") {
  if(is.infinite(time_limit)) {
    ...
    result <- vector(mode = 'list', length = length(objects))
    execution_times <- c()
